...
export default function FileCard({ file, onDownload, onDelete }: Props) {
  return (
    <div className="glass-card p-4 flex flex-col gap-3 hover:shadow-lg transition-shadow">
      ...
      <div className="flex gap-2 pt-1">
        {onDownload && (
          <button onClick={() => onDownload(file.file_id)} className="glass-button !py-1.5 !px-3 !text-xs flex items-center gap-1">
            <Download size={14} /> 下载
          </button>
        )}
        {onDelete && (
          <button onClick={() => onDelete(file.file_id)} className="glass-button !py-1.5 !px-3 !text-xs flex items-center gap-1 !bg-red-500/80">
            <Trash size={14} /> 删除
          </button>
        )}
      </div>
