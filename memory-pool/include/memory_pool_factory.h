...
inline void TieredMemoryPoolDeleter(MemoryPoolBase<TieredMemoryPool>* p) {
  // NOLINTNEXTLINE(cppcoreguidelines-owning-memory,cppcoreguidelines-pro-type-static-cast-downcast)
  // CRTP 静态 deleter 模式：基类无 virtual 析构，需 static_cast 到派生类后 delete
  delete static_cast<TieredMemoryPool*>(p);
}

using MemoryPoolPtr =
    std::unique_ptr<MemoryPoolBase<TieredMemoryPool>, decltype(&TieredMemoryPoolDeleter)>;
