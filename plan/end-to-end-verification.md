...
- [x] 新增 `virtual bool path_exists(std::string_view path) const = 0;`

### 3. net/http/include/router.h
- [x] 新增 `bool path_exists(std::string_view path) const override;`

### 4. net/http/src/router.cpp
- [x] 实现 `path_exists`（搜索 trie 忽略 method）

### 5. net/http/src/http_server.cpp
- [x] B5 修复: `handle_connection` 中 match 失败后调 `path_exists` → 405

### 6. verification/verify.sh — 验证脚本增强
- [x] V7: 10 次 Keep-Alive（原 3 次）
- [x] V12: 文件全生命周期（上传/元信息/下载/hash/大文件/重复上传/二进制）
- [x] V14: 4 线程并发请求
- [x] V15: 边界（空 body、PUT/DELETE 方法不匹配）
